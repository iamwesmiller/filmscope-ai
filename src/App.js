...
// Generate audience analysis based on film data
...
// Initial data and other generators...
const initialCampaigns = [
  { id: 'camp_1', title: 'Trailer Launch Teaser', content: 'First look at our new film! Trailer drops Friday.', platform: 'instagram', type: 'Video', status: 'Completed', metrics: { reach: 12500, engagement: 8.2, clicks: 450, cost: 50, roas: 3.1 }, metaAdStatus: 'Promoted', createdAt: new Date(2025, 5, 4) },
  { id: 'camp_2', title: 'Meet the Director Q&A', content: 'Our director will be live on Instagram this Wednesday at 8 PM EST.', platform: 'instagram', type: 'Community', status: 'Active', metrics: { reach: 8200, engagement: 12.5, clicks: 150, cost: 20, roas: 4.5 }, metaAdStatus: 'Promoted', createdAt: new Date(2025, 5, 5) },
];
...
const CampaignManager = ({ campaigns, setCampaigns, setNotification, setActiveTab }) => {
  ...
  const handleDelete = (campaign) => {
    setConfirmModal({
      ...
      onConfirm: async () => {
        if (!supabase?.from) {
          setNotification({ type: 'error', message: 'Supabase not configured.' });
          return;
        }
  ...
  const DashboardView = () => (
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
      <div className="xl:col-span-2 space-y-6">
        ...
        <AppCard className="p-6">
          <div className="flex justify-between items-center mb-4">
            <h3 className="font-bold text-lg text-white font-poppins">All Campaigns</h3>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-zinc-300">
                      {campaign.type} - 
                      <span className={`px-2 py-0.5 text-xs rounded-full ${campaign.status === 'Active' ? 'bg-green-500/20 text-green-300' : campaign.status === 'Scheduled' ? 'bg-blue-500/20 text-blue-300' : campaign.status === 'Draft' ? 'bg-yellow-500/20 text-yellow-300' : 'bg-gray-500/20 text-gray-300'}`}>
                        {campaign.status}
                      </span>
                    </p>
                  </div>
      ...
      <AppCard className="p-6">
        <h3 className="font-bold text-lg text-white mb-4 flex items-center font-poppins">
          <Bot className="mr-2" /> AI Optimization Engine
        </h3>
      </AppCard>
    </div>
  );
      ...
      <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
        <AppCard className="p-8 max-w-2xl w-full animate-fade-in-up">
          <div className="flex justify-between items-start mb-6">
            <h3 className="text-2xl font-bold text-white font-poppins">{campaign.id ? 'Edit Campaign' : 'Create New Campaign'}</h3>
  ...
  return (
    <div>
      <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" style={{ display: confirmModal.isOpen ? 'flex' : 'none' }}>
        <AppCard className="p-8 max-w-md w-full animate-fade-in-up">
          <h3 className="text-xl font-bold text-white mb-4 font-poppins">{confirmModal.title}</h3>
          <p className="text-zinc-300 mb-6">{confirmModal.message}</p>
          <div className="flex justify-end gap-4">
            <AppButton onClick={() => setConfirmModal({ isOpen: false })} variant="secondary">Cancel</AppButton>
            <AppButton onClick={confirmModal.onConfirm} variant="danger">Confirm</AppButton>
          </div>
        </AppCard>
      </div>
